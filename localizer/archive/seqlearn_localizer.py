...
class SeqlearnLocalizer(object):
    def __init__(self, subject_id):
        with open('seqlearn_localizer_config.yml') as f:
        ...
        # labels
        self.behav_data = np.loadtxt(self.ref_dir+'/trial-data.txt',delimiter=',',skiprows=1)
        self.trial_data = self.behav_data[::self.presses_per_trial,:]
        self.trial_targets = self.trial_data[:,-1]
        self.trial_chunks = self.trial_data[:,0]-1
    ...
    def save_classifier_as_nifti(self):
        ...
        def export_to_pycortex(self):
            import cortex
            cortex.freesurfer.import_subj(self.fs_subject_id, sname=self.subject_id)
            # cmd = 'tkregister2 --mov functionals/rfi.nii --targ anatomicals/raw.nii.gz --reg transforms/rai2rfi.dat --fslregout transforms/rai2rfi.mat --noedit'
            # os.system(cmd)
            from cortex.xfm import Transform
            x = np.loadtxt('transforms/rai2rfi.mat')
            Transform.from_fsl(x, 'functionals/rfi.nii', 'anatomicals/raw.nii.gz').save(self.subject_id, 'rai2rfi', 'coord')
